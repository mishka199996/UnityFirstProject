...
{
    //now what object are clickable
    public LayerMask clickableLayer;

    //Swap cursor per object
    public Texture2D pointer; //Normal pointer
    public Texture2D target; //Cursor for clickable object like world
    ...
    void Update()
    {
        ...
        if(Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit, 50, clickableLayer.value))
        {
            ...
            if(Input.GetMouseButtonDown(0))
            {
                ...
                {
                    Transform itemPas = hit.collider.gameObject.transform;

                    OnClickEnvironment.Invoke(itemPas.position);
                    Debug.Log("Item");
                }
                else
                {
